...
import {
  encodeMazeSnapshot,
  restoreMazeRuntime,
  type MazeSnapshotEnvelope,
} from '../../../games/ai-maze-escape/src/persistence/snapshot';
import { queueMazeInfluence } from '../../../games/ai-maze-escape/src/influence/apply';
import type { MazeInfluenceCommand } from '../../../games/ai-maze-escape/src/influence/types';
import { MazeRuntime } from '../../../games/ai-maze-escape/src/runtime/run';

export interface MazeChannelOptions {
  channelId: string;
  workerId: string;
  seed: string;
...
export interface MazeDependencyHealth {
  gateway: boolean;
  moderation: boolean;
  entitlement: boolean;
  audit: boolean;
...
export interface MazeChannelStatus {
  started: boolean;
  ...
  dependencies: MazeDependencyHealth;
...
export interface MazeRecoveryCheckpoint extends Omit<SnapshotRecord, 'envelope'> {
  envelope: MazeSnapshotEnvelope;
}

export type MazeRuntimeCommand =
  | { schemaVersion: 1; id: string; seq: number; kind: 'step' }
  | { schemaVersion: 1; id: string; seq: number; kind: 'restart'; seed: string }
  | { schemaVersion: 1; id: string; seq: number; kind: 'influence'; command: MazeInfluenceCommand };

export type MazeRecoveryResult =
  | {
      status: 'restored';
      runtime: MazeRuntime;
      snapshotId: string;
...
function checkpointBase(record: MazeRecoveryCheckpoint) {
  const { checksum: _, ...base } = record;
  return base;
}

function validCheckpoint(record: MazeRecoveryCheckpoint): boolean {
...
export function createMazeRecoveryCheckpoint(
  runtime: MazeRuntime,
  options: {
    streamId: string;
    id: string;
...
): MazeRecoveryCheckpoint {
  const envelope = encodeMazeSnapshot(runtime);
  const base = {
...
export function recoverMazeFromEvidence(input: {
  snapshots: ReadonlyArray<MazeRecoveryCheckpoint>;
  commands: ReadonlyArray<MazeRuntimeCommand>;
  compatibility: CompatibilityKey;
  ...
  for (const snapshot of candidates) {
    ...
    let runtime: MazeRuntime;
    try {
      runtime = restoreMazeRuntime(structuredClone(snapshot.envelope));
    } catch (error) {
...
export class MazeChannelService {
  public runtime: MazeRuntime;
  ...
  private dependencies: MazeDependencyHealth = {
    gateway: true,
    moderation: true,
    entitlement: true,
    audit: true,
  ...
  constructor(private readonly options: MazeChannelOptions) {
    ...
    this.runtime = MazeRuntime.create(options.config, options.seed);
  ...
  start(nowMs: number): MazeChannelStatus {
    ...
    if (this.hadDurableHistory) {
      ...
      ) as MazeRecoveryCheckpoint[];
      const synthetic = createMazeRecoveryCheckpoint(this.runtime, {
      ...
      const result = recoverMazeFromEvidence({
  ...
  enqueueInfluence(commandId: string, command: MazeInfluenceCommand, nowMs: number) {
  ...
  captureSnapshot(nowMs: number): MazeRecoveryCheckpoint {
    this.requireStarted();
    const record = createMazeRecoveryCheckpoint(this.runtime, {
      streamId: this.options.channelId,
  ...
  recover(input: { nowMs: number; newOwnerId: string; expectedChecksum?: string }): MazeRecoveryResult {
    this.requireStarted();
  ...
  setDependencyHealth(next: Partial<MazeDependencyHealth>): void {
  ...
  executeControl(command: ControlCommand, nowMs: number) {
    ...
    // Reserve a durable request record before any control-plane mutation. The
    // outcome record follows, but this first record guarantees an auditable
    // actor, action and payload even if a later write or process boundary fails.
    this.appendAudit(
  ...
  private applyCommand(
    ...
    input:
      ...
      | { kind: 'influence'; command: MazeInfluenceCommand },
  ) {
    this.requireStarted();
    ...
    if (input.kind === 'influence') {
      ...
      const queued = queueMazeInfluence(preparedInfluenceState, input.command);
    ...
    // Validation has completed before the append. Once this succeeds, the
    // command is guaranteed replayable and can safely mutate in-memory truth.
  ...
  private commandsFromStore(): MazeRuntimeCommand[] {
      ...
      .map(item => {
        ...
        if (item.payload.kind === 'influence') {
          return {
            ...
            command: structuredClone(item.payload.command) as MazeInfluenceCommand,
          };
        }
  ...
  private persistSemanticEvents(events: ReturnType<MazeRuntime['drainEvents']>, nowMs: number): void {
    for (const event of events) {
      const payload = { ...(event.data ?? {}), semanticSeq: event.seq };
      if (event.type === 'result') {
        Object.assign(payload, {
          steps: this.runtime.state.explorer.steps,
          discoveryPermille: this.runtime.state.result?.discoveryPermille ?? 0,
