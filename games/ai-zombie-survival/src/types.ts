...
export interface Survivor extends Vec2{id:string;name:string;role:SurvivorRole;action:SurvivorAction;health:number;stamina:number;infection:number;morale:number;ammo:number;carrying:number;carryingKind:keyof Resources|null;threat:number;targetX:number;targetY:number;facing:number;cooldown:number;kills:number;rescued:number;alive:boolean;goal:string;intent:string;confidence:number;fallback:boolean;planAgeTicks:number;stuckTicks:number;lastPlanX:number;lastPlanY:number;insideBuildingId:string|null;escortCivilianId:string|null}
