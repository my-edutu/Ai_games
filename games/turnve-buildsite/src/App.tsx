...
export function App() {
  ...
  useEffect(() => {
    const key = (event: KeyboardEvent) => {
      if (event.shiftKey && event.code === 'KeyP') { event.preventDefault(); state.setSelectedInteractable(null); setPresenterOpen((open) => !open); return; }
      if (event.code === 'Tab' && state.started && !['intro', 'ppe', 'briefing', 'report'].includes(state.stage)) { event.preventDefault(); state.setSelectedInteractable(null); setTabletOpen((open) => !open); }
      if (event.code === 'Escape') { state.setSelectedInteractable(null); setTabletOpen(false); setPresenterOpen(false); setHint(null); }
    };
    window.addEventListener('keydown', key);
    return () => window.removeEventListener('keydown', key);
  }, [state.started, state.stage, state.setSelectedInteractable]);
