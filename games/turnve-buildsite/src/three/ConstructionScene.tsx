...
function SiteEnvironment({ weather }: { weather: WeatherState }) {
  ...
  return (
    <>
      ...
      <mesh position={[-21, 1.4, 24]} onClick={choose('site-office')}><boxGeometry args={[9, 2.8, 0.45]} /><meshStandardMaterial color="#252a2d" /></mesh>
      <mesh position={[-21, 1.4, 23.72]} onClick={choose('site-office')}><boxGeometry args={[6.2, 1.1, 0.12]} /><meshStandardMaterial color="#e8ad31" /></mesh>
      <mesh position={[-12, 1.5, -11]} castShadow onClick={choose('site-office')}><boxGeometry args={[8, 3, 5]} /><meshStandardMaterial color="#d9d5ca" /></mesh>
      <mesh position={[10, 0.28, 0]} receiveShadow onClick={choose('slab')}><boxGeometry args={[15, 0.55, 11]} /><meshStandardMaterial color="#777975" roughness={.9} /></mesh>
      {[-5, 0, 5].flatMap((x) => [-4, 0, 4].map((z) => <mesh key={`${x}-${z}`} position={[10 + x, 2.7, z]} castShadow onClick={choose('slab')}><boxGeometry args={[0.6, 5.4, 0.6]} /><meshStandardMaterial color="#b5b2aa" /></mesh>))}
      {Array.from({ length: 7 }, (_, i) => <mesh key={`rebar-${i}`} position={[7 + i, 0.66, -1.8]} onClick={choose('rebar')}><boxGeometry args={[0.08, 0.08, 6]} /><meshStandardMaterial color="#60463c" /></mesh>)}
      ...
      <mesh position={[-12, 0.5, 8]} onClick={choose('cement-storage')}><boxGeometry args={[4, 1, 3]} /><meshStandardMaterial color={materialsProtected ? '#526f76' : '#c0a980'} /></mesh>
      ...
      <mesh position={[4, 0.12, -8]} rotation={[0, 0, Math.PI / 2]} onClick={choose('temporary-cable')}><cylinderGeometry args={[0.08, 0.08, 7, 10]} /><meshStandardMaterial color="#242627" /></mesh>
      <mesh position={[10, 0.7, -10]} onClick={choose('formwork')}><boxGeometry args={[7, 1.4, 0.25]} /><meshStandardMaterial color={hazards.formwork.status === 'resolved' ? '#47795e' : '#a77d50'} /></mesh>
