...
export function FormworkInteraction() {
  ...
  return <group position={[10, 0, -10]} name="formwork-direct-practice">
    ...
    {/* One physical correction at a time prevents ambiguous hit targets and mirrors real work sequencing. */}
    {mentor.stepIndex === 4 && !propSeated && <WorldDragPractice position={[-.55, 1.55, .75]} testId="formwork-prop-drag" label="Reseat prop on bearing" onProgress={setPropProgress} onComplete={(_, progress) => {
    ...
    {mentor.stepIndex === 4 && propSeated && !braceAttached && <WorldDragPractice position={[.7, 1.58, .75]} testId="formwork-brace-drag" label="Attach brace to anchor" onProgress={setBraceProgress} onComplete={(_, progress) => {
      ...
      dispatch({
        type: 'COMPLETE_STEP',
        actionType: 'correct-support',
        quality: score.quality,
        interaction: { kind: 'attach', metrics: { ...score.metrics, propSeated: 1, braceAttached: 1 } },
      });
    }} />}
