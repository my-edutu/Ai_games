  ...
  return (
    <div className="report-backdrop">
      <article className="final-report">
        ...
        <section className="report-employer-card"><span className="eyebrow dark">EMPLOYER EVIDENCE SNAPSHOT</span><h2>What this run proves</h2><p>The score is supported by an audit trail of what the learner observed, documented, communicated and escalated—not only their final answer.</p><ul><li><b>{state.evidence.length}</b> captured evidence item{state.evidence.length === 1 ? '' : 's'} linked to site observations</li><li><b>{artifactCount}/4</b> professional artifacts submitted and evaluated</li><li><b>{stakeholderContacts}</b> stakeholder relationship{stakeholderContacts === 1 ? '' : 's'} received documented information</li><li><b>{safeAuthority ? 'Authority respected' : state.reworkRisk ? 'Critical authority failure recorded' : 'Authority evidence incomplete'}</b> — intern-level approval boundaries were explicitly assessed</li><li><b>{report.consequenceChain.length}</b> decision/consequence event{report.consequenceChain.length === 1 ? '' : 's'} preserved in the cause-and-effect audit trail</li></ul></section>
