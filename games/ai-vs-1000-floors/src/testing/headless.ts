...
import{FLOORS_DETERMINISTIC_VERSION}from '../version';
...
export interface FloorsHeadlessReport{
  schemaVersion:1;gameId:'ai-vs-1000-floors';deterministicVersion:typeof FLOORS_DETERMINISTIC_VERSION;runs:FloorsHeadlessRun[];totalTicks:number;
...
export function runFloorsHeadless(options:FloorsHeadlessOptions):FloorsHeadlessReport{
  ...
  const body={schemaVersion:1 as const,gameId:'ai-vs-1000-floors' as const,deterministicVersion:FLOORS_DETERMINISTIC_VERSION,runs,totalTicks,outcomes,invariantFailures,replayFailures,generatorInvalid};
