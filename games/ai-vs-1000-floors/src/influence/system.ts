...
export function reverseFloorsInfluence(source:FloorsState,id:string):{status:'reversed'|'not-found';state:FloorsState}{const state=clone(source),inf=data(state),queued=inf.queued.find(q=>q.id===id);if(queued){queued.reversed=true;return{status:'reversed',state}}if(Object.prototype.hasOwnProperty.call(inf.applied,id))return{status:'reversed',state};return{status:'not-found',state:source}}
...
export function resolveFloorsVote(source:FloorsState,voteId:string,tick:number,rng:NamedRng):{status:'resolved'|'not-ready'|'missing';state:FloorsState;winner:FloorsInfluenceEffectId}{const state=clone(source),inf=data(state),vote=inf.votes?.[voteId];if(!vote)return{status:'missing',state:source,winner:'route-scan'};if(tick<vote.closesAtTick)return{status:'not-ready',state:source,winner:vote.options[0]};const counts=new Map(vote.options.map(o=>[o,0]));for(const choice of Object.values(vote.votes))counts.set(choice,(counts.get(choice)??0)+1);const max=Math.max(...counts.values()),tied=vote.options.filter(o=>(counts.get(o)??0)===max).sort(),winner=tied[rng.nextInt(`audience-tiebreaks:${vote.id}`,tied.length)];vote.resolved=true;const queued=submitFloorsInfluence(state,{id:`${vote.id}:result`,effectId:winner,source:'vote',receivedAtTick:state.tick,policyVersion:'floors-influence-v1'});return{status:'resolved',state:queued.status==='accepted'?queued.state:state,winner}}
