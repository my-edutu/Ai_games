...
export type FloorsPolicy='fallback'|'wait-test';
...
export class FloorsRuntime{
  public state:FloorsState;
  public rng:NamedRng;
  private events:FloorsEvent[];
  private readonly rootSeed:string;
  private runOrdinal:number;
  private readonly policy:FloorsPolicy;
  ...
  static create(configInput:Partial<FloorsConfig>,seed:string,options:FloorsRuntimeOptions={}):FloorsRuntime{
    const config=validateFloorsConfig(configInput),runId=options.runId??`${seed}:run:0`,rng=NamedRng.fromSeed(seed),state=createFloorsInitialState(config,seed,runId,rng);
    const runtime=new FloorsRuntime({state,rng:rng.snapshot(),events:[],rootSeed:seed,runOrdinal:0,policy:options.policy??'fallback'});
    runtime.emit('runtime-initialized',{gameId:'ai-vs-1000-floors',runId:state.runId});runtime.emit('floor-started',{floor:1,sector:1,archetype:state.floor.archetype});return runtime;
  }
  ...
  private emit(type:string,data?:Record<string,unknown>):void{this.state.eventSequence++;this.events.push({seq:this.state.eventSequence,tick:this.state.tick,type,data})}
  private restart():void{
    this.runOrdinal++;const seed=`${this.rootSeed}:restart:${this.runOrdinal}`,runId=`${this.rootSeed}:run:${this.runOrdinal}`;this.rng=NamedRng.fromSeed(seed);this.state=createFloorsInitialState(this.state.config,seed,runId,this.rng);this.events=[];this.emit('runtime-restarted',{runId,runOrdinal:this.runOrdinal});this.emit('floor-started',{floor:1,sector:1,archetype:this.state.floor.archetype});
  }
  ...
  step(action?:FloorsAction):FloorsState{
    ...
    const decision:FloorsDecision=action?{action,mode:'operator',confidence:'high',intent:'Applying a validated explicit action.',reason:'explicit-action',expansions:0}:this.policy==='wait-test'?{action:{kind:'wait'},mode:'test',confidence:'high',intent:'Waiting for deterministic timeout coverage.',reason:'wait-test',expansions:0}:chooseFallbackAction(this.state);
    const prepared=clone(this.state);prepared.ai.mode=decision.mode==='fallback'?'fallback':'tactical';prepared.ai.goal='Reach the tower exit';prepared.ai.intent=decision.intent;prepared.ai.confidence=decision.confidence;prepared.ai.decisions++;prepared.ai.nodeExpansions=decision.expansions;if(decision.mode==='fallback')prepared.ai.fallbackCount++;prepared.ai.lastPlanChangeReason=decision.reason;
    const result=applyFloorsAction(prepared,decision.action,this.rng);
    if(!result.accepted){this.emit('action-rejected',{reason:result.reason,action:decision.action});return this.state}
    this.state=result.state;for(const pending of result.events)this.emit(pending.type,pending.data);return this.state;
  }
