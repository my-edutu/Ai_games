...
function nearestTarget(state:FloorsState):number{
  if((state.floor.objective==='defeat-warden'||state.floor.objective==='defeat-architect')&&state.floor.enemies.length){
    return [...state.floor.enemies].sort((a,b)=>manhattan(state.player.cell,a.cell,state.floor.width)-manhattan(state.player.cell,b.cell,state.floor.width)||a.id.localeCompare(b.id))[0].cell;
  }
...
export function chooseProductionAction(state:FloorsState):FloorsDecision{
  ...
  if(route.reached&&route.path.length>1){const next=route.path[1],move=legal.find(a=>a.kind==='move'&&a.targetCell===next);if(move&&threat(state,next)<=7)return{action:move,mode:'tactical',confidence:threat(state,next)<=2?'high':'medium',intent:target===state.floor.exit?'Advancing toward the verified exit.':'Closing on the sector guardian.',reason:'threat-aware-route',expansions:route.expansions}}
  const rewardMoves=legal.filter(a=>a.kind==='move'&&a.targetCell!==undefined&&state.floor.rewardCells.includes(a.targetCell)).sort((a,b)=>threat(state,a.targetCell)-threat(state,b.targetCell)||actionKey(a).localeCompare(actionKey(b)));
  if(rewardMoves[0]&&threat(state,rewardMoves[0].targetCell)<=3)return{action:rewardMoves[0],mode:'tactical',confidence:'medium',intent:'Taking a low-risk upgrade opportunity.',reason:'safe-reward',expansions:route.expansions};
  const safeMoves=legal.filter(a=>a.kind==='move'&&a.targetCell!==undefined).sort((a,b)=>threat(state,a.targetCell)-threat(state,b.targetCell)||manhattan(a.targetCell,target,state.floor.width)-manhattan(b.targetCell,target,state.floor.width)||actionKey(a).localeCompare(actionKey(b)));
  if(safeMoves[0])return{action:safeMoves[0],mode:'tactical',confidence:'medium',intent:'Repositioning to reduce incoming pressure.',reason:'lowest-threat-move',expansions:route.expansions};
