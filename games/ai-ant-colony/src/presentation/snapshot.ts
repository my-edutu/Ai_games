...
const SAFE_EVENT_KEYS = new Set([
  'reason', 'score', 'population', 'band', 'strategy', 'weather', 'season', 'role',
  'amount', 'store', 'kind', 'count', 'health', 'total', 'brood', 'id', 'antId',
  'predatorId', 'day', 'pressure', 'remaining', 'runIndex', 'effectId', 'magnitude',
  'spawned', 'theme', 'untilTick', 'foodAdded', 'waterAdded',
]);
...
function boundedText(value: unknown, max = 96) {
...
function lifecycleRevision(state: AntColonyState) {
  if (state.lifecycle === 'active') return 1;
  if (state.lifecycle === 'result') return 2;
  if (state.lifecycle === 'intermission') {
    return 100 + Math.max(0, state.config.intermissionTicks - state.intermissionRemaining);
  }
  return 9000;
}
...
function sceneFor(state: AntColonyState): AntPublicScene {
  ...
  if (
    state.colony.threat >= 60 ||
    state.predators.length >= 2 ||
    state.queen.health <= Math.ceil(state.config.queenHealth * .35)
  ) return 'crisis';
  return 'colony';
}

function headlineFor(state: AntColonyState, scene: AntPublicScene) {
...
function captionForEvent(event: AntPublicEvent) {
  switch (event.type) {
    case 'result': return event.data.reason === 'ascension'
      ? 'The colony reached ecological ascension.'
      : 'The colony cycle has ended.';
    ...
    case 'influence-applied': return `Audience effect applied: ${String(event.data.effectId ?? 'bounded influence')}.`;
...
export function createAntRenderSnapshot(
  state: AntColonyState,
  events: ReadonlyArray<AntEvent> = [],
): Readonly<AntRenderSnapshot> {
  ...
  const captions = recentEvents
    ...
    .slice(-4);
  if (!captions.length) {
    captions.push(`Strategy: ${boundedText(state.colony.strategyReason, 112) || state.colony.strategy}.`);
  }

  const broodByStage = { egg: 0, larva: 0, pupa: 0 };
  for (const brood of state.brood) broodByStage[brood.stage]++;

  const dayLength = Math.max(1, state.config.dayLengthTicks);
  const seasonLength = Math.max(1, state.config.seasonLengthTicks);
  const dayProgress = (state.tick % dayLength) / dayLength;
  const seasonProgress = (state.tick % seasonLength) / seasonLength;
  ...
  const snapshot: AntRenderSnapshot = {
    ...
    queen: {
      health: state.queen.health,
      maxHealth: state.config.queenHealth,
      eggsLaid: state.queen.eggsLaid,
    },
    colony: {
      population: state.ants.length,
    ...
    audience: {
      enabled: state.influence.enabled && !state.influence.emergencyDisabled,
