...
function clearMantleSpace(p:TowerState['player'],platform:TowerPlatform,all:TowerPlatform[]){const targetX=clamp(p.position.x,platform.x+p.halfWidth+1,platform.x+platform.width-p.halfWidth-1),top=platform.y+platform.height,targetY=top+p.halfHeight,l=targetX-p.halfWidth,r=targetX+p.halfWidth,b=targetY-p.halfHeight,t=targetY+p.halfHeight;return!all.some(other=>other.id!==platform.id&&other.kind!=='oneway'&&overlap1(l,r,other.x,other.x+other.width)&&overlap1(b,t,other.y,other.y+other.height))}
export function stepTowerPhysics(input:TowerState,action:TowerAction):{state:TowerState;contacts:PhysicsContact[];integrityFailure?:string}{
  ...
  if(!Number.isInteger(state.stats.wallJumps))state.stats.wallJumps=0;if(!Number.isInteger(state.stats.mantles))state.stats.mantles=0;
  ...
  const targetX=p.position.x+p.velocity.x;let newX=targetX,wallNormalX=0,mantled=false,mantlePlatform:TowerPlatform|undefined;
  for(const platform of nextPlatforms){if(platform.kind==='oneway')continue;const pb=p.position.y-p.halfHeight,pt=p.position.y+p.halfHeight;if(!overlap1(pb,pt,platform.y,platform.y+platform.height))continue;
    const oldL=p.position.x-p.halfWidth,oldR=p.position.x+p.halfWidth,newL=newX-p.halfWidth,newR=newX+p.halfWidth,top=platform.y+platform.height,playerTop=p.position.y+p.halfHeight,nearTop=playerTop>=top-6000&&playerTop<=top+16000,slowVertical=Math.abs(p.velocity.y)<=7000;
    if(p.velocity.x>0&&oldR<=platform.x&&newR>=platform.x){
      if(!p.grounded&&!action.wallJump&&nearTop&&slowVertical&&clearMantleSpace(p,platform,nextPlatforms)){newX=platform.x+p.halfWidth+1;p.position.y=top+p.halfHeight;p.velocity={x:0,y:0};p.grounded=true;p.groundedPlatformId=platform.id;p.jumpCharges=1;p.state='mantling';state.stats.mantles++;mantled=true;mantlePlatform=platform;contacts.push({kind:'land',entityId:platform.id,normal:{x:0,y:1}});break}
      newX=platform.x-p.halfWidth;p.velocity.x=0;wallNormalX=-1;contacts.push({kind:'wall',entityId:platform.id,normal:{x:-1,y:0}})
    }
    else if(p.velocity.x<0&&oldL>=platform.x+platform.width&&newL<=platform.x+platform.width){
      if(!p.grounded&&!action.wallJump&&nearTop&&slowVertical&&clearMantleSpace(p,platform,nextPlatforms)){newX=platform.x+platform.width-p.halfWidth-1;p.position.y=top+p.halfHeight;p.velocity={x:0,y:0};p.grounded=true;p.groundedPlatformId=platform.id;p.jumpCharges=1;p.state='mantling';state.stats.mantles++;mantled=true;mantlePlatform=platform;contacts.push({kind:'land',entityId:platform.id,normal:{x:0,y:1}});break}
      newX=platform.x+platform.width+p.halfWidth;p.velocity.x=0;wallNormalX=1;contacts.push({kind:'wall',entityId:platform.id,normal:{x:1,y:0}})
    }
  }
  if(!mantled){p.position.x=clamp(newX,p.halfWidth,c.worldWidth-p.halfWidth);if(p.position.x!==newX){p.velocity.x=0;wallNormalX=newX<0?1:-1;contacts.push({kind:'wall',entityId:'world-boundary',normal:{x:wallNormalX,y:0}})}}else p.position.x=clamp(newX,p.halfWidth,c.worldWidth-p.halfWidth);
  let wallJumped=false;if(!mantled&&!p.grounded&&action.wallJump&&wallNormalX!==0){const impulse=Math.min(c.dashSpeed,Math.max(c.maxRunSpeed,Math.round(c.jumpImpulse*.6)));p.velocity.x=wallNormalX*impulse;p.velocity.y=c.jumpImpulse;p.facing=wallNormalX as -1|1;p.grounded=false;p.groundedPlatformId=undefined;p.state='wall-jumping';state.stats.wallJumps++;wallJumped=true}
  const targetY=p.position.y+p.velocity.y;let newY=targetY,landed:TowerPlatform|undefined=mantlePlatform;
  if(!mantled){for(const platform of nextPlatforms){const left=p.position.x-p.halfWidth,right=p.position.x+p.halfWidth;if(!overlap1(left,right,platform.x,platform.x+platform.width))continue;
  ...
  }}else newY=p.position.y;
  p.position.y=newY;p.grounded=Boolean(landed);p.groundedPlatformId=landed?.id;if(mantled){p.jumpCharges=1;p.state='mantling'}else if(landed){p.jumpCharges=1;if(p.state!=='hurt')p.state='standing'}else if(wallJumped){p.state='wall-jumping'}else if(p.state!=='dashing'&&p.state!=='hurt')p.state='airborne';
  ...
  state.height=Math.max(0,p.position.y);if(state.height>state.stats.maxHeight){state.stats.maxHeight=state.height;state.meaningfulEventTick=state.tick}
  if(old.x===p.position.x&&old.y===p.position.y&&action.move===0&&!action.jump&&!action.wallJump)p.velocity.x=0;
