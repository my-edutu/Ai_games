...
export interface TowerAction{move:-1|0|1;jump:boolean;wallJump:boolean;dash:boolean;attack:boolean;ability:boolean;upgradeChoice?:string}
...
export interface TowerStats{floorsCleared:number;maxHeight:number;jumps:number;wallJumps:number;mantles:number;dashes:number;falls:number;hazardHits:number;invalidActions:number;restarts:number;checkpoints:number;technicalOutcomes:number;enemiesDefeated:number;guardiansDefeated:number;damageDealt:number;damageTaken:number;pickupsCollected:number;upgradesApplied:number;stuckRecoveries:number;projectilesFired:number}
