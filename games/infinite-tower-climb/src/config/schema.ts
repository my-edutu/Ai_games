...
export interface TowerConfig{
  ...
  maxPlatformsPerChunk:number;maxHazardsPerChunk:number;playerHalfWidth:number;playerHalfHeight:number;
  gravity:number;moveAcceleration:number;maxRunSpeed:number;groundFriction:number;jumpImpulse:number;dashSpeed:number;
  maxFallSpeed:number;killDepth:number;launchFloor:number;level:number;
}
const defaults:TowerConfig={
  ...
  maxPlatformsPerChunk:12,maxHazardsPerChunk:8,playerHalfWidth:9000,playerHalfHeight:14000,
  gravity:1200,moveAcceleration:900,maxRunSpeed:9000,groundFriction:1300,jumpImpulse:15000,dashSpeed:17000,
  maxFallSpeed:22000,killDepth:120000,launchFloor:0,level:1
};
...
export function parseTowerConfig(input:Partial<TowerConfig>={}):TowerConfig{
  ...
  integer('launchFloor',c.launchFloor,0,1000000);integer('level',c.level,1,1000000);
