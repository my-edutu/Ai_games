...
function emptyResult(player: PlayerState): KinematicStepResult {
  return { player, contacts: [], landed: false, jumpStarted: false, slideStarted: false, vaultStarted: false, stumbleStarted: false, failed: false };
}
function normalizeLegacyState(player: PlayerState): void { if (player.movementState === "airborne") player.movementState = player.velocity.y > 0 ? "rising" : "falling"; }

export function stepPlayerKinematic(source: PlayerState, route: RouteState, intent: PlayerControlIntent, config: EkoRunConfig, tick: number): KinematicStepResult {
  ...
  const acceleration = groundLike ? (intent.axis === 0 ? config.groundDeceleration : config.groundAcceleration) : config.airAcceleration;
  ...
  if (groundLike && player.movementState !== "rising") {
    const support = sampleSupportSurface(route, player.position.x, config.playerHalfWidth, tick, config, player.position.y + config.maxStepHeight);
...
export function integrateFoundationMovement(source: PlayerState, route: RouteState, axis: number, config: EkoRunConfig): PlayerState {
  return stepPlayerKinematic(source, route, { axis, jumpPressed: false, jumpReleased: false, slide: false, vault: false }, config, 0).player;
}
