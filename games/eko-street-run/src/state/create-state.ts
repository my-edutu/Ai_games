...
export class InvariantError extends Error {
  ...
  constructor(code: string, message: string) { super(message); this.name = "InvariantError"; this.code = code; }
}

const MOVEMENT_STATES = new Set<MovementState>(["grounded", "rising", "falling", "sliding", "vaulting", "stumbling", "dead", "airborne"]);
...
function createInitialPlayer(route: RouteState): PlayerState {
  return {
    position: { x: route.startX, y: route.groundY }, velocity: { x: 0, y: 0 }, movementState: "grounded", facing: 1,
    coyoteTicksRemaining: 0, jumpBufferTicksRemaining: 0, jumpCutConsumed: false,
...
function createStateForRoute(config: EkoRunConfig, route: RouteState, withHazards: boolean): EkoRunState {
  ...
  const state: EkoRunState = {
    schemaVersion: config.schemaVersion, gameVersion: config.gameVersion, deterministicVersion: config.deterministicVersion,
    contentVersion: config.contentVersion, runId: config.runId, rootSeed: config.seed, tick: 0, nextEventSequence: 0,
    lifecycle: "running", player: createInitialPlayer(route), route,
    ...(withHazards ? { hazards: createHazardRuntimeState(config.seed) } : {}),
    resources: { ekoTokens: 0 }, commandWatermarks: {}, randomStreams: streams.snapshotAuthoritative(),
...
export function createInitialState(config: EkoRunConfig): EkoRunState { return createStateForRoute(config, createFoundationRoute(), false); }
export function createPhase5State(config: EkoRunConfig): EkoRunState { return createStateForRoute(config, createPhase5Route(), true); }
...
export function createPhase6State(config: EkoRunConfig): EkoRunState {
  ...
  const state: EkoRunState = {
    ...
    contentVersion: config.contentVersion, runId: config.runId, rootSeed: config.seed, tick: 0, nextEventSequence: 0,
    lifecycle: "running", player: createInitialPlayer(route), route,
    hazards: createHazardRuntimeStateFromContracts(progression.activeContent.hazards), progression,
    resources: createPhase6Resources(), commandWatermarks: {}, randomStreams: streams.snapshotAuthoritative(),
    record: { maxProgress: 0, completedTick: null },
  };
  assertStateInvariants(state, config.maxCommandSources, config.playerHalfWidth);
  return state;
...
export function cloneState(state: EkoRunState): EkoRunState { return JSON.parse(JSON.stringify(state)) as EkoRunState; }
...
function assertTimer(value: number, name: string): void { if (!Number.isInteger(value) || value < 0 || value > 10_000) throw new InvariantError("INVALID_TIMER", `${name} must be a bounded non-negative integer`); }
function assertOptionalTick(value: number | null, name: string): void { if (value !== null && (!Number.isInteger(value) || value < 0)) throw new InvariantError("INVALID_HAZARD_TICK", `${name} must be null or a non-negative integer`); }

function assertRoute(route: RouteState): void {
  if (!(route.minX < route.maxX) || route.startX < route.minX || route.startX > route.maxX || route.finishX > route.maxX) throw new InvariantError("INVALID_ROUTE_BOUNDS", "route bounds are invalid");
  ...
  const addId = (id: string): void => { if (!id || ids.has(id)) throw new InvariantError("DUPLICATE_ROUTE_ID", "route geometry IDs must be non-empty and unique"); ids.add(id); };
  for (const segment of route.groundSegments) { addId(segment.id); if (segment.minX > segment.maxX) throw new InvariantError("INVALID_GROUND_SEGMENT", "ground segment bounds are invalid"); }
  for (const slope of route.slopes) { addId(slope.id); if (!(slope.minX < slope.maxX)) throw new InvariantError("INVALID_SLOPE", "slope bounds are invalid"); }
  for (const collider of route.colliders) {
    ...
    if (collider.motion && (!Number.isInteger(collider.motion.periodTicks) || collider.motion.periodTicks < 2 || collider.motion.minOffsetX > collider.motion.maxOffsetX)) throw new InvariantError("INVALID_COLLIDER_MOTION", "moving collider motion contract is invalid");
  }
}
...
function assertHazards(state: EkoRunState): void {
  ...
  for (const encounter of state.hazards.encounters) {
    ...
    assertOptionalTick(encounter.warningTick, "warningTick"); assertOptionalTick(encounter.resolvedTick, "resolvedTick");
...
function assertResourceArray(value: string[] | undefined, name: string): void {
  if (!value) return;
  if (value.length > PHASE6_LEDGER_ID_LIMIT || new Set(value).size !== value.length || value.some(item => typeof item !== "string" || item.length < 1 || item.length > 128)) throw new InvariantError("INVALID_RESOURCE_LEDGER", `${name} must be unique and bounded`);
}
...
function assertPhase6(state: EkoRunState): void {
  ...
  if (!Number.isInteger(state.resources.earnedTokenTotal) || (state.resources.earnedTokenTotal ?? -1) < state.resources.ekoTokens || (state.resources.earnedTokenTotal ?? PHASE6_TOKEN_CAP + 1) > PHASE6_TOKEN_CAP) throw new InvariantError("INVALID_TOKEN_AUDIT", "lifetime token earnings must be bounded and cover the current balance");
...
export function assertStateInvariants(state: EkoRunState, maxCommandSources = 8, playerHalfWidth = PLAYER_HALF_WIDTH): void {
  assertFiniteTree(state); assertRoute(state.route); assertHazards(state); assertPhase6(state);
  ...
  assertTimer(state.player.coyoteTicksRemaining, "coyoteTicksRemaining"); assertTimer(state.player.jumpBufferTicksRemaining, "jumpBufferTicksRemaining");
  assertTimer(state.player.landingCompressionTicksRemaining, "landingCompressionTicksRemaining"); assertTimer(state.player.slideTicksRemaining, "slideTicksRemaining"); assertTimer(state.player.stumbleTicksRemaining, "stumbleTicksRemaining");
  if (state.player.vault) { assertTimer(state.player.vault.ticksRemaining, "vault.ticksRemaining"); assertTimer(state.player.vault.totalTicks, "vault.totalTicks"); if (state.player.vault.totalTicks < 1 || state.player.vault.ticksRemaining > state.player.vault.totalTicks) throw new InvariantError("INVALID_VAULT", "vault timing is inconsistent"); }
  ...
  const minimumBodyCenter = state.route.minX + playerHalfWidth; const maximumBodyCenter = state.route.maxX - playerHalfWidth;
