...
function publicHazards(state: EkoRunState): PublicHazardSnapshot[] {
  ...
  return getHazardContractsForState(state).map(contract => ({
    id: contract.id, family: contract.family, x: hazardPositionAtTick(contract, state.tick), y: contract.y,
    width: contract.width, height: contract.height, active: hazardActiveAtTick(contract, state.tick),
    phase: byId.get(contract.id)?.phase ?? "unseen", legalResponses: [...contract.legalResponses],
    captionKey: contract.captionKey, visualToken: contract.visualToken,
  }));
}
...
export function createRenderSnapshot(state: EkoRunState, recentEvents: readonly SemanticEvent[] = []): Readonly<EkoRunRenderSnapshot> {
  ...
  const snapshot: EkoRunRenderSnapshot = {
    ...
    player: {
      ...
      movementState: state.player.movementState, facing: state.player.facing,
    ...
    ...(progression ? {
      progression: {
        districtIndex: progression.districtIndex, districtId: progression.districtId, cycle: progression.cycle,
        districtCompletions: progression.districtCompletions, totalDistance: progression.totalDistance,
        pacingBand: progression.pacingBand, difficulty: { ...progression.activeContent.difficulty },
      ...
      resources: {
        ...
        earnedTokenTotal: state.resources.earnedTokenTotal ?? state.resources.ekoTokens,
