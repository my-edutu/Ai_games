...
export function assessReadiness(input:ReadinessInput):Readonly<ReadinessResult>{
  ...
  const uniqueFailures=[...new Set(failures)].sort(),uniqueBlockers=[...new Set(blockers)].sort(),acceptedRisks=[...new Set([...(input.findings.acceptedP2??[]),...(review.acceptedP2??[])])].sort();const verdict:ReadinessResult['verdict']=uniqueFailures.length?'FAIL':uniqueBlockers.length?'BLOCKED':'PASS';const implementationPass=['manifest','traceability','campaign'].every(g=>gateStatus[g]==='pass')&&input.capacity.performancePass&&input.drills.implementationStatus==='pass'&&uniqueFailures.length===0;const highestTruthfulReadiness=verdict==='PASS'?'R5':implementationPass?'R4':'R3';const base={verdict,highestTruthfulReadiness,productionReady:verdict==='PASS',blockers:uniqueBlockers,failures:uniqueFailures,acceptedRisks,gateStatus};const result={...base,assessmentChecksum:JSON.stringify(base)};return deepFreeze(result)as Readonly<ReadinessResult>;
}
