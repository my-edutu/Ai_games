...
test('manifest exposes stable identity, versions, tick rate and runtime entry point',()=>{assert.equal(zombieManifest.gameId,'ai-zombie-survival');assert.equal(zombieManifest.gameVersion,'0.2.0');assert.equal(zombieManifest.deterministicVersion,'zombie-v4');assert.equal(zombieManifest.tickRate,10);assert.equal(zombieManifest.runtimeExport,'ZombieRuntime');assert.ok(Object.isFrozen(zombieManifest))});
