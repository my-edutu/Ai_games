...
test('release manifest freezes exact candidate and version identities',()=>{const manifest=buildFloorsReleaseManifest({candidateSha:'a'.repeat(40),createdAt:'2026-09-07T00:00:00Z',commands:['npm test','npm run test:browser'],artifactDigests:{tests:'deadbeef'}});assert.equal(manifest.candidateSha,'a'.repeat(40));assert.equal(manifest.deterministicVersion,'floors-r1-v2');assert.equal(manifest.contentVersion,'floors-content-v1');assert.equal(manifest.presentationVersion,'floors-presentation-r3');assert.equal(manifest.influenceVersion,'floors-influence-v1');assert.match(manifest.manifestDigest,/^[0-9a-f]{8}$/);assert.throws(()=>buildFloorsReleaseManifest({...manifest,candidateSha:'short'}),/candidateSha/)});
