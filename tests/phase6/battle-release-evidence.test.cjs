...
test('Phase 5, Phase 6, final review and R5 evidence-intake documents exist for Game 6',()=>{const required=['../../games/ai-battle-royale/phases/PHASE-05-RELIABILITY-OPERATIONS.md','../../games/ai-battle-royale/phases/PHASE-06-RELEASE-VALIDATION.md','../../docs/reviews/AI_BATTLE_ROYALE_FINAL_REVIEW.md','../../docs/release/AI_BATTLE_ROYALE_R5_EVIDENCE_INTAKE.md'];for(const relative of required){const file=path.resolve(__dirname,relative);assert.equal(fs.existsSync(file),true,file);assert.ok(fs.readFileSync(file,'utf8').length>500,file)}});
