...
test('tower desktop broadcast is animated readable and privacy safe',async({page})=>{
  ...
  const diagnostics=await waitForVisibleWorld(page);expect(diagnostics.playerVisible).toBe(true);expect(diagnostics.platformsVisible).toBeGreaterThan(0);expect(diagnostics.sampleCount).toBeLessThanOrEqual(120);expect(Number.isFinite(diagnostics.frameMsP95)).toBe(true);
...
test('phone landscape retains game progress intent captions and visible gameplay',async({page})=>{
  ...
  const diagnostics=await waitForVisibleWorld(page);expect(diagnostics.playerVisible).toBe(true);expect(diagnostics.platformsVisible).toBeGreaterThan(0);await page.screenshot({path:path.join(artifacts,'phone-landscape.png'),fullPage:true});
});
...
test('phone portrait retains health stamina AI intent and visible gameplay',async({page})=>{
  ...
  const diagnostics=await waitForVisibleWorld(page);expect(diagnostics.playerVisible).toBe(true);expect(diagnostics.platformsVisible).toBeGreaterThan(0);
...
test('reduced motion high contrast mute and clean feed preserve the game view',async({page})=>{
  ...
  const diagnostics=await waitForVisibleWorld(page);expect(diagnostics.playerVisible).toBe(true);await page.screenshot({path:path.join(artifacts,'clean-feed.png'),fullPage:true});
});
...
const evidenceCases=[
  ...
  ['guardian','guardian-encounter.png',s=>s.enemies.some(e=>e.active&&e.kind==='guardian'),d=>d.playerVisible&&d.guardiansVisible>0],
...
for(const[scenario,name,predicate,visiblePredicate]of evidenceCases){
  test(`visual evidence ${scenario} keeps required gameplay subjects on screen`,async({page})=>{
    ...
    const diagnostics=await page.evaluate(()=>window.__TOWER_RENDER_DIAGNOSTICS__);expect(visiblePredicate(diagnostics),`${scenario} required subjects must be visibly framed`).toBeTruthy();expect(diagnostics.sampleCount).toBeLessThanOrEqual(120);expect(Number.isFinite(diagnostics.frameMsP95)).toBe(true);
