...
test('escape room desktop broadcast is a physical Three.js room, readable and privacy safe',async({page})=>{
  ...
  await expect(page.locator('[data-testid="objective"]')).toContainText(/escape/i);await expect(page.locator('[data-testid="ai-intent"]')).not.toBeEmpty();await expect(page.locator('[data-testid="room-title-index"]')).toContainText('ROOM 01');await expect(page.locator('[data-testid="room-title-name"]')).not.toBeEmpty();
