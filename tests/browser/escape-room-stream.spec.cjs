...
async function present(page,frame,{inspectId=null,room=false}={}){await page.evaluate(({frame,inspectId,room})=>{window.__ESCAPE_PRESENT_FRAME__(frame);if(room)window.__ESCAPE_ROOM_VIEW__();else if(inspectId)window.__ESCAPE_INSPECT_OBJECT__(inspectId)}, {frame,inspectId,room});await page.waitForTimeout(420)}

test('escape room desktop broadcast is a physical Three.js room, readable and privacy safe',async({page})=>{
  ...
  const canvas=page.locator('[data-testid="escape-canvas"]');await expect(canvas).toBeVisible();await expect(canvas).toHaveAttribute('data-renderer','three-physical-room-v2');
  await expect(page.locator('[data-testid="objective"]')).toContainText(/escape/i);await expect(page.locator('[data-testid="ai-intent"]')).not.toBeEmpty();await page.waitForFunction(()=>Boolean(window.__ESCAPE_PUBLIC_STATE__));const titleState=await page.evaluate(()=>window.__ESCAPE_PUBLIC_STATE__);await expect(page.locator('[data-testid="room-title-index"]')).toHaveText(`ROOM ${String(titleState.roomIndex).padStart(2,'0')}`);await expect(page.locator('[data-testid="room-title-name"]')).not.toBeEmpty();
  ...
  const state=await page.evaluate(()=>window.__ESCAPE_PUBLIC_STATE__);expect(state).toBeTruthy();expect(state.renderVersion).toBe('escape-render-v2');expect(state.seed).toBeUndefined();expect(state.runId).toBeUndefined();expect(state.roomSeed).toBeUndefined();expect(JSON.stringify(state)).not.toMatch(/hiddenFact|solution|oracle/i);expect(state.objects.every(object=>object.placement&&typeof object.placement.surface==='string')).toBe(true);
  await page.waitForFunction(()=>window.__ESCAPE_RENDER_DIAGNOSTICS__?.renderer==='three-physical-room-v2');const diagnostics=await page.evaluate(()=>window.__ESCAPE_RENDER_DIAGNOSTICS__);expect(diagnostics.renderer).toBe('three-physical-room-v2');expect(diagnostics.engine).toBe('three');expect(diagnostics.webgl).toBe(true);expect(diagnostics.roomSurfaces).toBeGreaterThanOrEqual(5);expect(diagnostics.physicalProps).toBeGreaterThan(20);expect(diagnostics.materialFamilies).toBeGreaterThanOrEqual(12);expect(diagnostics.meshFamilies).toBeGreaterThanOrEqual(8);expect(diagnostics.themedDressing).toBeGreaterThan(0);expect(diagnostics.adaptiveQuality).toBe(true);expect(['high','medium','low']).toContain(diagnostics.qualityTier);expect(['bounded-pcf','disabled-quality-tier']).toContain(diagnostics.shadowMode);expect([18,36,54]).toContain(diagnostics.particleCount);expect(diagnostics.fps).toBeGreaterThan(0);expect(diagnostics.drawCalls).toBeLessThan(190);expect(diagnostics.triangles).toBeLessThan(150000);fs.writeFileSync(path.join(artifacts,'runtime-diagnostics.json'),JSON.stringify(diagnostics,null,2));
...
test('runtime replay produces the required physical-room evidence sequence',async({page})=>{
  test.setTimeout(60000);await page.setViewportSize({width:1920,height:1080});await page.goto(`${base}/escape-room?muted=1&evidence=1`,{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>Boolean(window.__ESCAPE_PUBLIC_STATE__&&window.__ESCAPE_PRESENT_FRAME__&&window.__ESCAPE_INSPECT_OBJECT__));
  ...
  const danger=sameRun.find(frame=>frame.scene==='danger');if(danger){await present(page,danger,{room:true});const dangerDiagnostics=await page.evaluate(()=>window.__ESCAPE_RENDER_DIAGNOSTICS__);expect(dangerDiagnostics.hazardVisuals).toBeGreaterThan(0);await page.screenshot({path:path.join(artifacts,'hazard-state.png'),fullPage:true});}
