'use strict';
const fs=require('node:fs');const path=require('node:path');const{test,expect}=require('@playwright/test');const base='http://127.0.0.1:4175',artifacts=path.resolve(__dirname,'../../artifacts/escape-room-phase3');test.beforeAll(()=>fs.mkdirSync(artifacts,{recursive:true}));
...
test('escape room desktop broadcast is a physical WebGL room, readable and privacy safe',async({page})=>{
  ...
  const diagnostics=await page.evaluate(()=>window.__ESCAPE_RENDER_DIAGNOSTICS__);expect(diagnostics.renderer).toBe('webgl-physical-room-v1');expect(diagnostics.webgl).toBe(true);expect(diagnostics.roomSurfaces).toBeGreaterThanOrEqual(5);expect(diagnostics.physicalProps).toBeGreaterThan(10);expect(diagnostics.materialFamilies).toBeGreaterThanOrEqual(5);expect(diagnostics.shadowMode).toBe('contact-proxy');expect(diagnostics.particleCount).toBe(18);expect(diagnostics.fps).toBeGreaterThan(0);
