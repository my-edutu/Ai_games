...
const state={snapshot:null,eventSource:null,reconnectAttempt:0,lastEventSequence:-1,audio:null,audioEnabled:true,selectedTileIndex:null};
...
function render(snapshot){
  state.snapshot=snapshot;text('subtitle',snapshot.headline.subtitle);text('era-clock',`Year ${snapshot.time.year} · Day ${snapshot.time.day}`);text('tier-chip',snapshot.progress.tier);text('goal',snapshot.goal.name);text('decree',`${snapshot.goal.decree} · ${snapshot.goal.pressure}`);
